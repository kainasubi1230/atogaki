...
def _local_handwritten_variant(
    ...
    *,
    ...
    for s in strokes:
        ...
        local_rot_deg = rng.uniform(-3.5, 3.5)
        local_scale = rng.uniform(0.95, 1.05)
        local_tx = rng.uniform(-0.9, 0.9)
        local_ty = rng.uniform(-0.9, 0.9)

        # For short "dot-like" strokes, randomize orientation a bit stronger.
        if short_dot:
            local_rot_deg += rng.uniform(-26.0, 26.0)
            local_scale *= rng.uniform(0.86, 1.18)
            local_tx += rng.uniform(-0.5, 0.5)
            local_ty += rng.uniform(-0.5, 0.5)
        ...
        period = rng.uniform(0.8, 1.4)
        wobble_amp = rng.uniform(0.15, 0.55)
        if short_dot:
    ...
    *,
    ...
    for ch in joyo:
        ...
        for i in range(nvar):
            ...
            strokes = _local_handwritten_variant(strokes, rng=var_rng)
            ...
            rows.append(
                {
                    ...
                    "meta": {
                        ...
                        "svg_file": code,
                        "variant": i,
                        "profile": "kanjivg_local_jitter_v3_compact",
