...
def main():
    ...
    # Print counts
    dark_count = int(smoothed.sum()) # foreground True (which becomes 0 in tile)
    bright_count = int((~smoothed).sum()) # background False (which becomes 255 in tile)
    
    print(f"Bbox size: {smoothed.shape[1]}x{smoothed.shape[0]} (total: {smoothed.size})")
    print(f"Foreground pixels (dark): {dark_count}")
    print(f"Background pixels (bright): {bright_count}")
    print(f"Is dark_count <= bright_count? {dark_count <= bright_count}")
    print(f"Would _image_to_sequence invert mask? {not (dark_count <= bright_count)}")
