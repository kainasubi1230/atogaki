...
### アセットの自動ダウンロード（ゼロ構成セットアップ）

リポジトリを新規クローンした際、動作に必要な大容量アセットが `storage/` 配下に存在しない場合、`api` コンテナの起動時（`on_startup` イベント）に自動で指定のURLからアセットをダウンロードし、セットアップを行います。これにより、手動でファイルを配置しなくてもすぐにシステムを使用開始できます。

- **自動ダウンロードの対象ファイル**:
  - `storage/models/base_model.pt` （学習済みベースモデル）
  - `storage/base_hiragana_katakana_exemplars.json` （基本文字軌跡データセット）

- **カスタマイズ用の環境変数** (デフォルトでは公開アセットのデフォルトURLが適用されます):
  - `BASE_MODEL_URL`: ベースモデル（`.pt`）のダウンロード先URL
  - `BASE_EXEMPLARS_URL`: 基本データセット（`.json`）のダウンロード先URL

*※ もしネットワーク制限などで自動ダウンロードに失敗した際、ローカルの `storage/models/base_model.pt.bak` や `storage/base_hiragana_katakana_exemplars.json.bak` にバックアップファイルが存在していれば、自動的にそこからコピーして復元するフォールバック処理も内包されています。*
